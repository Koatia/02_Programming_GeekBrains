...
public class Service {
    ...
    public boolean addFirstHumanToFamily() {
        ...
        human = new HumanBuilder().setBirthDate(LocalDate.of(1945, 5, 28)).setDeathDate(LocalDate.of(2021, 8, 22))
                .setName("Ivan").setGender(Gender.Male).build();
        familyTree.add(human);

        human = new HumanBuilder().setBirthDate(LocalDate.of(1950, 1, 1)).setDeathDate(LocalDate.of(2021, 2, 19))
                .setSpouse(familyTree.getById(0)).setName("Zoia").setGender(Gender.Female).build();
        familyTree.add(human);

        human = new HumanBuilder().setBirthDate(LocalDate.of(1973, 12, 13)).addParent(familyTree.getById(0))
                .addParent(familyTree.getById(1)).setName("Kostia").setGender(Gender.Male).build();
        familyTree.add(human);

        human = new HumanBuilder().setBirthDate(LocalDate.of(1975, 3, 6)).setSpouse(familyTree.getById(2))
                .setName("Irina").setGender(Gender.Female).build();
        familyTree.add(human);

        human = new HumanBuilder().setBirthDate(LocalDate.of(2001, 11, 29)).addParent(familyTree.getById(2))
                .addParent(familyTree.getById(3)).setName("Katia").setGender(Gender.Female).build();
        familyTree.add(human);
    ...
    public int addNewHumanToFamily(String name, Gender gender, LocalDate birthDate) {
        ...
        human = new HumanBuilder().setBirthDate(birthDate).setName(name).setGender(gender).build();
